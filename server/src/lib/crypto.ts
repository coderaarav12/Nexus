...
// ---------- private folder unlock tokens (short-lived, HMAC-signed) ----------

export function signPrivateUnlock(uid: number): string {
  const payload = b64url(
    Buffer.from(
      JSON.stringify({ uid, purpose: 'private_unlock', exp: Date.now() + 30 * 60000 }),
    ),
  );
  const sig = crypto.createHmac('sha256', config.tokenSecret).update(`punlock.${payload}`).digest('base64url');
  return `punlock.${payload}.${sig}`;
}

export function verifyPrivateUnlock(token: string): { uid: number } | null {
  try {
    const [prefix, payload, sig] = token.split('.');
    if (prefix !== 'punlock') return null;
    const expected = crypto.createHmac('sha256', config.tokenSecret).update(`punlock.${payload}`).digest('base64url');
    if (!crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expected))) return null;
    const data = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
    if (data.purpose !== 'private_unlock' || data.exp < Date.now()) return null;
    return { uid: data.uid as unknown as number };
  } catch {
    return null;
  }
}
...
// ---------- per-user data encryption keys (private folders) ----------
// A DEK is a random 32-byte key. It is wrapped with masterKey for storage.
// Content is encrypted with AES-256-GCM using the DEK (a fresh IV per blob).
...
export function wrapDek(dek: Buffer): string {
  ...
  const tag = cipher.getAuthTag();
  return `w1:${iv.toString('base64')}:${tag.toString('base64')}:${enc.toString('base64')}`;
}
...
// blob format: p1:<iv b64>:<tag b64>:<ciphertext b64>
export function encryptPrivateBlob(dek: Buffer, plain: Buffer): string {
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', dek, iv);
  const enc = Buffer.concat([cipher.update(plain), cipher.final()]);
  const tag = cipher.getAuthTag();
  return `p1:${iv.toString('base64')}:${tag.toString('base64')}:${enc.toString('base64')}`;
}

export function decryptPrivateBlob(dek: Buffer, stored: string): Buffer {
  const [v, ivB64, tagB64, dataB64] = stored.split(':');
  if (v !== 'p1') throw new Error('unknown private blob format');
